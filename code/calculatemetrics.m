function hmetrics = calculatemetrics(npcdata, seglength, varargin)
% calculatemetrics Custom Canopy/Terrain height metrics calculation
%
%   hmetrics = calculatemetrics(npcdata, seglength) calculates basic canopy
...
%   9 or more points with max. height >= 2.0 meter. The output, hmetrics,
%   also contains associated point stats (segment total, canopy and terrain
%   totals) and segment geolocation data,- mid lat/lon, begin and end lat/lon.
%   Table may also include various data flags, if included in prior processes.
%
%
%   hmetrics = calculatemetrics(npcdata, seglength, canopy_percentiles) will
...
%
%   hmetrics = calculatemetrics(npcdata, seglength, canopy_percentiles, minpts)
%   limits canopy height calculation to segments with minpts points or more
%
%   hmetrics = calculatemetrics(npcdata, seglength, canopy_percentiles,
...
if isempty(canopy_percentiles)
    ...
    pc_fields = makepcFields(canopy_percentiles);% percentiles var names
    pc_flag = true;
end
...
hmetrics = zeros(length(edg)+10,length(varNames));
k = 1;
...
for b = 2:length(edg)
    ...
    if ~isempty(flag_data)
        ...
        hmetrics(k,:) = [k mid_atd mid_ll lon1 lat1 lonout latout totpts capts tepts hgt seg_flags];
    else
        hmetrics(k,:) = [k mid_atd mid_ll lon1 lat1 lonout latout totpts capts tepts hgt];
    end
...
hmetrics = array2table(hmetrics,'VariableNames',varNames);
hmetrics.Properties.Description = 'Canopy/Terrain Height Metrics';
hmetrics = addprop(hmetrics,{'type'},{'table'});
hmetrics.Properties.CustomProperties.type = 'hmetrics';

end

function pcFields = makepcFields(pc)
pc = round(pc,0);
formatSpec = "h_ca_p%d";
s = compose(formatSpec,pc);
